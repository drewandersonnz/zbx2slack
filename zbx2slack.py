...
class NoticeInfo(object):
    '''
    NoticeInfo build Slack object for sending
    '''
    ...
    def _gen_items(self):
        ...
        _items = [
            ...
            for i in [
                item_text.split('|')
                for item_text in self._item_text_list if not r'*UNKNOWN*' in item_text
            ]
        ]
...
def main():
    ...
    # Environment Check and merge to params
    # -------------------------------
    params = {
        'ZABBIX_SERVER_URL': '',
    ...
    for env in params.keys():
        if env in os.environ.keys():
            params[env] = os.environ[env]
    ...
    #     --slack-incoming-webhook-url "https://hooks.slack.com/services/xxxx/xxxx/.." \
    ...
    parser.add_argument('--zabbix-server-url',
                        default=params['ZABBIX_SERVER_URL'],
                        help='Your Zabbix server URL (Default: "{0}")'.format(
                            params['ZABBIX_SERVER_URL']),
                        type=str)
    ...
    parser.add_argument('--slack-incoming-webhook-url',
                        default=params['INCOMING_WEBHOOK_URL'],
                        help='Slack Bot name (Default: "{0}")'.format(
                            params['INCOMING_WEBHOOK_URL']),
                        type=str)
    ...
    parser.add_argument('--item', action='append',
                        type=str,
                        help='Set Zabbix Macro formated by'
                             '"{HOST.NAME1}|{ITEM.NAME1}|{ITEM.KEY1}|{ITEM.VALUE1}|{ITEM.ID1}"')
